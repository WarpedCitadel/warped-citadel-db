...
readme rip_deploy_local deploy_local: localpgip    :=$(LOCAL_PG_IP)
readme rip_deploy_local deploy_local: localpgport  :=$(LOCAL_PG_PORT)
database_rip rip_deploy_local rip_dev rip_qa: psql :=$(LOCAL_PG_PSQL)
...
\n\
	rip_deploy_local	Drop and recreate LOCAL databases at $(localpgip):$(localpgport).\n\
...
database_rip:
	...
	cd $(pgdir); /
	$(psql) --hostname=$(pghost) -p $(pgport) --username=postgres --dbname=postgres -a -f $(pgfile) -v vbname='$(pgdbname)'
